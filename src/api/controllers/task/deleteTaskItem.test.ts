...
describe("DELETE /v1/tasks/{taskUid}/items/{itemUid}", function () {

    const SNAPSHOT_FILE = path.join(__dirname, "../../../../../snapshots/", `task.snap`);

    it("should delete task item", async () => {

        const res = await chaiRequest("DELETE", `/v1/tasks/${task1.uid}/items/${taskItem1.uid}`, accessToken1.token);
    ...
    it("should fail deleting non existing task", async () => {
        ...
        const path = `/v1/tasks/${task1.uid}/items/954f261d-cd52-4070-8e96-b942b4b44a6d`;
