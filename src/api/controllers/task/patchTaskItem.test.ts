...
describe("PATCH /v1/tasks/{taskUid}/items", function () {
    ...
    it("should patch task item entity", async () => {
        ...
        const res = await chaiRequest("PATCH", `/v1/tasks/${task1.uid}/items/${taskItem1.uid}`, accessToken1.token)
            .send(payload);
    ...
    it("should fail patching non existing task item", async () => {
        ...
        const res = await chaiRequest("PATCH", `/v1/tasks/${task1.uid}/items/${taskItemUid}`, accessToken1.token)
            .send(payload);
