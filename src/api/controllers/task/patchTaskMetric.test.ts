...
describe("PATCH /v1/tasks/{taskUid}/metrics", function () {
    ...
    it("should patch task metric entity", async () => {
        ...
        const res = await chaiRequest("PATCH", `/v1/tasks/${task1.uid}/metrics/${taskMetric1.uid}`, accessToken1.token)
            .send(payload);
    ...
    it("should fail patching non existing task metric", async () => {
        ...
        const res = await chaiRequest("PATCH", `/v1/tasks/${task1.uid}/metrics/${taskMetricUid}`, accessToken1.token)
            .send(payload);
